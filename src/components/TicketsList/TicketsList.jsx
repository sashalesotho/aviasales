...
const TicketList = ({ tickets, checkAll, withoutTrans, oneTrans, twoTrans, threeTrans, cheapestTab, ticketsToShow }) => {
	...
	const createList = (toShow) => {
		if (cheapestTab) {
			tickets.sort((value1, value2) => value1.price - value2.price);
		}

		if (!cheapestTab) {
			tickets.sort((value1, value2) => 
		...
		if (filtered.length === 0) {
			return (
				<li>
					<Alert className={classes.alert} message='' type='info' showIcon />
				</li>
			);
		}
		return filtered;
